...
const CloudAuth = {
    ...
    //
    // Supabase 대시보드의 Redirect URLs 허용 목록에 현재 주소가 없으면 Supabase가 redirectTo를
    // 무시하고 Site URL(홈페이지)로 보내버릴 수 있다. 홈페이지의 안전장치 스크립트가 이 값을
    // 읽어 다시 이 페이지로 돌려보내준다.
    ...
    // 마이그레이션의 project_invites_target_check 참고).
    // 반환: { ok: true } | { ok: false, error: string }
    async inviteMember(ownerUserId, projectName, identifier, role = 'editor') {
        const user = await this.getUser();
        ...
        if (invitedEmail) {
            // 이메일 초대는 기존 유니크 제약(owner_id,project_name,invited_email)으로 upsert.
            const { error } = await _supabase
            ...
            // 아이디 초대는 invited_email이 null이라 위 유니크 제약이 걸리지 않는다
            // (NULL은 유니크 인덱스에서 서로 다른 값으로 취급) — 기존 pending 초대를
            // 직접 지우고 새로 꽂아 beat_song_invites와 동일하게 중복을 방지한다.
            await _supabase
                .from('project_invites')
                .delete()
                .eq('owner_id', ownerUserId)
    ...
    // 이메일 매칭과 아이디(invited_user_id) 매칭을 OR로 봐야 하는데, PostgREST의 .or()
    // 문자열 필터는 이메일에 콤마/괄호가 섞이면 이스케이프가 필요해 번거로워서, 대신
    // 두 조건을 따로 조회해 합치는 방식으로 안전하게 처리한다.
    async listReceivedInvites() {
        const user = await this.getUser();
        if (!user) return [];
        const [{ data: byEmail, error: e1 }, { data: byHandle, error: e2 }] = await Promise.all([
            _supabase.from('project_invites')
        ...
        if (e1) console.warn('listReceivedInvites(email) 오류:', e1.message);
        if (e2) console.warn('listReceivedInvites(handle) 오류:', e2.message);
        const invites = [...(byEmail || []), ...(byHandle || [])]
    ...
    async listSentInvites(ownerUserId, projectName) {
        ...
        const nickMap = idTargets.length ? await this._fetchNicknameMap(idTargets) : {};
        return invites.map(inv => ({
            ...inv,
